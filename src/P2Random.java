...
public class P2Random {
    ...
    public static final int MAX_FORCE_SENSITIVITY = 100;
    public static final int MAX_QUANTITY = 50;

    /**
     * Generate random deployments for Project 2
     * @param seed the initial seed for the random number generator
     * @param numGenerals total number generals, producing values [0, numGenerals)
     * @param numPlanets total number of planets, producing values [0, numPlanets)
     * @param numDeployments total number of random deployments to generate
     * @param arrivalRate the average number of deployments per timestamp
     * @return a {@link Scanner} object ready to read the deployments
     */
    public static Scanner PRInit(int seed, int numGenerals, int numPlanets, int numDeployments, int arrivalRate) {
        Random rd = new MTRandom(true);
        rd.setSeed(seed);

        AtomicReference<Double> timestamp = new AtomicReference<>( 0d);

        return new Scanner(
                Stream.generate( () -> {
                    int time_increase = (int) (Integer.toUnsignedLong(rd.nextInt()) % arrivalRate + 1);
                    timestamp.updateAndGet(v -> (double) (v + (1.0d / time_increase)));

                    int planetNum = (int) (Integer.toUnsignedLong(rd.nextInt()) % numPlanets);
                    int generalNum = (int) (Integer.toUnsignedLong(rd.nextInt()) % numGenerals);

                    String forceSide = (int) Integer.toUnsignedLong(rd.nextInt()) % 2 != 0 ? "SITH" : "JEDI";
                    int quantity = (int) (Integer.toUnsignedLong(rd.nextInt()) % MAX_QUANTITY + 1);
                    int forceSensitivity = (int) (Integer.toUnsignedLong(rd.nextInt()) % MAX_FORCE_SENSITIVITY + 1);

                    return String.format("%d %s G%d P%d F%d #%d",
                            timestamp.get().intValue(),
                            forceSide,
                            generalNum,
                            planetNum,
                            forceSensitivity,
                ...
                }).limit(numDeployments).collect(Collectors.joining("\n"))
        );
    }
